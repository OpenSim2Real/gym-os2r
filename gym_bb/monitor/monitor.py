...
class MonitorPlot(Wrapper):
    ...
    def __init__(self, env, plot_path=None, episodes_for_refresh=5, save_every_num_epidoes=10):
...
class VecMonitorPlot(VecEnvWrapper):
    ...
    def step_wait(self):
        ...
        if new_ep and self.epcount % self.episodes_for_refresh == 0:
            self.dplt.show()
        if new_ep and self.epcount % self.save_every_num_epidoes == 0 and self.saving:
            self.dplt.save_fig(self.plot_path)
...
# 			"no monitor files of the form *%s found in %s" % (VecMonitor.EXT, dir))
#          			dfs=[]
#          			headers=[]
